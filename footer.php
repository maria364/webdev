...
<script type="text/javascript">
	...
	<script type="text/javascript">
		function SendEmail(usname) {
			var usname;
			var y = confirm("Είστε σίγουροι πως θέλετε να αποστέλεται e-mail ενημέρωσης στον χρήστη " +usname + "?" );
			var check = 6;
		  ...
		  if (x == true)
			...
			$.ajax({
						url: "update.php",
						type: "POST",
						data: {  'username':usname, 'check': check },      //to data emperiexei times logikis {key:value}. to key einai auto pou pairnei tin timi apo ti metavliti kai ti metaferei ston php kwdika. p.x. ston php kwdika tha baloume "$_POST['key']"             
						success: function(data){
							alert(data); // this will print you any php / mysql error as an alert  
						}
					});	
